...
const Home = () => {
  ...
  return (
    ...
    {AllPost.map((e)=>
          {
            return <div className="grid-container"><HomeAllPost userName={e.userName} key={e.id+e.user} id={e._id} image={e.image} likes={e.likes} allPost={allPost} comment={e.comment}/></div>
          })
        }
    </div>
    </div>
